...
function ENT:Initialize()
	self:SetModel("models/cs16/w_thighpack.mdl")
...
function ENT:DoPickup( m_hClient )
	if m_hClient:Team() == TEAM_T then return false end
	if m_hClient:HasDefuser() then return false end
	if self.cantPickup then return false end

	self.cantPickup = true
	m_hClient:Setm_bHasDefuser( true )
